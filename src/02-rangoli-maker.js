...
export function extractRangoliCenter(design, start, end) {
  ...
  const result = design.slice(start,end);
  return result
}
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  const splittedArray = colorString.split(oldSep);
  const newjoinedArray = splittedArray.join(newSep);
...
export function makeRangoliBorder(char, length) {
  ...
  const repeatedDesign = char.repeat(length);
  return repeatedDesign.slice(0, length);
}
